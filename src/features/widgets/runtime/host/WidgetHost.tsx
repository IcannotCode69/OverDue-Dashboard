...
// If the file does not exist, create 'googleClient.ts' in 'src/features/calendar/api/' and export googleCalendarClient from it.
import { CalendarSettings } from '../../../calendar/types/index';
...
export default function CalendarCardSettings({ isOpen, onClose, anchorElement }: CalendarCardSettingsProps) {
  ...
  const loadSavedCredentials = () => {
    try {
      ...
      if (saved) {
        ...
        // If we have credentials but no env vars, show the form
        if (!process.env.REACT_APP_GOOGLE_CLIENT_ID && (creds.clientId || creds.apiKey)) {
  ...
  const saveCredentials = (newCredentials: { clientId: string; apiKey: string }) => {
    try {
      ...
      // Update the Google Calendar client with new credentials
      (googleCalendarClient as any).updateCredentials?.(newCredentials);
    } catch (error) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        // Show credentials form if connection fails due to missing config
        setShowCredentialsForm(true);
      }
    } catch (error) {
  ...
  const handleCredentialsSubmit = (e: React.FormEvent) => {
    ...
    setShowCredentialsForm(false);
    // Re-initialize the client with new credentials and try to connect
